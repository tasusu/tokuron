...
\begin{frame}[label=flow-example]{例}
    ...
    頂点の数字: \textcolor{UniGreen}{$b$}

    \begin{center}
    \begin{tikzpicture}[scale=1.6]

        \graph[no placement, mygraph]{
            s[at={(0,0)},  "$+7$" {UniGreen, left }];
            a[at={(2, 1)}, "$0$"  {UniGreen, above}];
            b[at={(2,-1)}, "$0$"  {UniGreen, below}];
            c[at={(4, 1)}, "$0$"  {UniGreen, above}];
            d[at={(4,-1)}, "$-2$" {UniGreen, below}];
            t[at={(6, 0)}, "$-5$" {UniGreen, right}];
        };
        \foreach \i/\j/\f/\u in {
            s/a/5/5,
            s/b/2/3,
...
\begin{frame}{最小費用流の整数性}
    主問題(P)の係数行列は$G$のネットワーク行列であり，したがって完全単模行列である．

    完全単模行列とLPの整数性（第1回参照）から，以下の定理が従う．

    \bigskip
    \begin{theorem}
        枝容量$u$と境界$b$がともに整数値だとする．
        このとき，最小費用流問題が実行可能であれば，整数値の最小費用流が存在する．
    \end{theorem}
\end{frame}
...
\begin{frame}{最短路問題 $\subset$ 最小費用流}
    ...
    \begin{tikzpicture}[xscale=.7]
        ...
        \node[UniGreen, left=1pt of s]{$-1$};
        \node[UniGreen, right=1pt of t]{$+1$};
    \end{tikzpicture}
    \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{輸送問題と最小費用流}
    ...
    \begin{column}{.35\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \node[right=1pt of 2, font=\footnotesize, align=center] {$u(\Out(j)) - b(j)$};
        \end{tikzpicture}
    \end{column}
    \end{columns}
    \bigskip
...
\begin{frame}[t]{相補性条件}
    ...
    \structure{$y \geq 0$:}
    $\varphi(a) > 0$ のとき，$a^{-1}$が$G_\varphi$に存在するので，ポテンシャルの定義より
    \setlength{\abovedisplayskip}{3pt}
    ...
    $\varphi(a) > 0$のときは，$y$の定義から自明に制約を満たす．$\varphi(a) = 0$のときは，$a$が$G_\varphi$に存在するので，ポテンシャルの定義より
    \[ 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}<1-2>[label=algs]{アルゴリズム}
    以下のアルゴリズムを紹介する:

    \begin{itemize}[<alert@+(1)>]
        \item 逐次最短路法 
        {\footnotesize\color{gray} \dotfill 重み付き二部マッチングの自然な拡張}
        \item 主双対法
        {\footnotesize\color{gray} \dotfill 重み付き二部マッチングの自然な拡張}
        \item 容量スケーリング
        {\footnotesize\color{gray} \dotfill NEW!}
    \end{itemize}
...
% \begin{frame}{例：逐次最短路法}
%     \centering
%     \begin{tikzpicture}[scale=2]
%         \graph[mygraph, no placement, math nodes]{
%             1[at={(0,0)}];
%             2[at={(1,1)}];
%             3[at={(1,-1)}];
%             4[at={(2,0)}];
%         };
%         \node[UniGreen, left=5pt of 1] {\small $+2$};
%         \node[UniGreen, right=5pt of 4] {\small $-2$};
%         \foreach \i/\j/\u/\c in {1/2/2/4, 1/3/2/2, 2/3/1/2, 2/4/3/3, 3/4/1/5} {
%             \draw[->] (\i) -- node[above, sloped] {\footnotesize $/\u; \c$} (\j);
%         }
%     \end{tikzpicture}
% \end{frame}
% \end{document}
...
\begin{frame}[t]{逐次最短路法の証明}
    ...
    \bigskip
    \structure{(∵)} 
    負閉路最適性条件より，$G_{\varphi^+}$に負閉路は存在しないことを示せば良い．

    $G_{\varphi^+}$に負閉路$C$が存在したと仮定．更新前の$G_\varphi$には負閉路$C$はなかったので，$C$の逆向き枝が更新時の最短路$P$に含まれていたことになる（下図）．

    \begin{center}
        $G_\varphi$
        \begin{tikzpicture}
            \draw[blue, thick, ->] decorate [decoration={coil, aspect=0}] {(0, 0) node {} to (6, 0) node[above] {$P$}};
            \draw[AlertOrange, thick, ->] (4, 0 ) -> (3, 0); 
            \draw[AlertOrange, thick, ->] (3, 0 ) -> (3, -1);
            \draw[AlertOrange, thick, ->] (3, -1) ->(4, -1) node[midway, above] {$C$};
            \draw[AlertOrange, thick, ->] (4, -1) -> (4, 0);
        \end{tikzpicture}
    \end{center}
    
    すると，迂回路$P'$で$P$より長さが短いものが存在し，矛盾． \qed

    \vfill
    \footnotesize
    \structure{(注)}
    次に紹介する主双対法の証明では，$(G_\varphi, \ell_\varphi)$にポテンシャルの存在が示される．「$\not\exists$負閉路 $\iff$ $\exists$ポテンシャル」より，ここで証明したことの別証明を与えている．
\end{frame}
...
\begin{frame}[t]{逐次最短路法の証明}
    ...
    \bigskip
    $G_\varphi$において$X$から出る枝は存在しないことから，以下が成り立つ:
    \begin{itemize}
...
\begin{frame}[t]{逐次最短路法の証明}
    \small
    また，$U^+, U^-$の定義から，以下が成り立つ: 
    \begin{itemize}
        \item $\partial\varphi(i) < b(i)$ \quad ($i \in U^+$) 
        \item $\partial\varphi(i) \leq b(i)$ \quad ($i \in V \setminus U^-$)
    \end{itemize}
    ...
    \begin{align*}
        b(X) 
        &= b(U^+) + b(X \setminus U^+) \\
        &> (\partial\varphi)(U^+) + (\partial\varphi)(X \setminus U^+) \tag{上の不等式} \\
        &= (\partial\varphi)(X) \\
        &= \varphi(\Out_G(X)) - \varphi(\In_G(X)) \\
        &= u(\Out_G(X)) - 0 \tag{$X$から出る枝がないことから}
    \end{align*}
...
\begin{frame}{主双対法の証明}
    ...
    \begin{lemma}
    主双対法における$p$は，常に$(G_\varphi, \ell_\varphi)$上のポテンシャル．
    \end{lemma}
    \pause
    \structure{(証明)} 
    % $p^+$は$(G_\varphi, \ell_\varphi)$のポテンシャルである．
    $p^+ := p + d$とする．{\footnotesize ($d: V \to \R$ ... $(G_\varphi, \ell_{\varphi,p})$上の（$U^+$からの）最短路長関数)}

    \bigskip
    \structure{Claim.} $p^+$は$(G_\varphi, \ell_\varphi)$のポテンシャル．
    ...
    \end{align*}
    が成り立つ．移項すると $p^+(j) - p^+(i) \leq \ell_\varphi(a)$. \qed
    }

    \pause
    \bigskip
    \structure{Claim.} 更新後の流$\varphi^+$に対し，$p^+$は$(G_{\varphi^+}, \ell_{\varphi^+})$のポテンシャル．
    ...
    \end{align*}
    よって，$\varphi$の更新により$P$上に逆向き枝が生じても，$p^+$がポテンシャルであることは保たれる． \qed }
\end{frame}
...
\begin{frame}{容量スケーリング法}
    ...
    \begin{itemize}
        ...
        \item 流$\varphi$に対し，
        \begin{align*}
            U^+_\Delta &:= \{i \in U^+ : b(i) \geq (\partial\varphi)(i) \highlight{$+ \Delta$} \} \subseteq U^+ \\
            U^-_\Delta &:= \{i \in U^- : b(i) \leq (\partial\varphi)(i) \highlight{$- \Delta$} \} \subseteq U^-
        \end{align*}
        \dotfill \alert{$\Delta$以上}供給が余っている/需要が満たされていない頂点
    \end{itemize}
    $\varphi$が整数流の場合: $\Delta = 1 \implies (U^+_\Delta, U^-_\Delta) = (U^+, U^-)$.
\end{frame}
...
\begin{frame}{容量スケーリング法の解析}
    ...
    \begin{itemize}
        ...
        \item $\Delta$フェーズにおける流$\varphi(a)$は$\Delta$の倍数． \\
        ...
        \item 流$\varphi$は容量制約$0 \leq \varphi \leq u$を満たす． \\
...
\begin{frame}[t]{容量スケーリング法の解析}
    ...
    \begin{column}{.7\textwidth}
    \structure{(証明)} 
    $\varphi$: $\Delta$フェーズの開始時の流 \\
    $X$: $G_\varphi$において$U^+_{2\Delta}$から到達可能な頂点全体 \\
    とする．このとき，
    \begin{itemize}
        \item $U^+_{2\Delta} \subseteq X \subseteq V \setminus U^-_{2\Delta}$. 
        \item $u \equiv +\infty$より，$G$に$X$から出る枝は存在しない．
    \end{itemize}
    \end{column}
    \begin{column}{.3\textwidth}
        \begin{tikzpicture}
            \draw[rounded corners, gray, fill=gray!20] (0, 0) rectangle ++(1, 2) node[above left] {$U^+_\Delta$};
            \draw[rounded corners, gray, fill=gray!40, very thick] (0, 0) rectangle ++(1, 1) node[midway] {$U^+_{2\Delta}$};
            \draw[rounded corners, gray, fill=gray!20] (3, 1) node[below right] {$U^-_{\Delta}$} rectangle ++(1, 2) ;
            \draw[rounded corners, gray, fill=gray!40, very thick] (3, 2) rectangle ++(1, 1) node[midway] {$U^-_{2\Delta}$};
            \draw[mysubset=AlertOrange, very thick, opacity=0.5] (0, 0) rectangle ++(4, 1.5) ++(0,-1.5) node[right, opacity=1.0] {$X$};

            \onslide<2->{
            \draw[blue, ->, thick] (0.5, 0.5) node[point, fill=blue, "{$s_\ell$}" left]{} 
            to[bend left, "{$P$}" above] 
            (3.5, 1.7) node[point, "{$t_\ell$}" right]{};
            }
        \end{tikzpicture}
    \end{column}
    \end{columns}
...
\begin{frame}[t]{容量スケーリング法の解析}
    ...
    \begin{align*}
        k
        &= \abs{\{\ell : s_\ell, t_\ell \in X\}} \\ 
        &\quad + \abs{\{\ell : s_\ell \in X, t_\ell \notin X\}} + \abs{\{\ell : s_\ell \notin X, t_\ell \in X\}} \\
        &\qquad + \abs{\{\ell : s_\ell, t_\ell \notin X\}} \\

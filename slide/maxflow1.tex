...
\begin{frame}
    ...
    \pause
    \structure{(証明)}
    枝容量$u \equiv 1$として$s$--$t$最大流を考えると，整数の$s$--$t$流は辺素$s$--$t$パスに対応する．整数性より，整数の$s$--$t$最大流が存在する．Mengerの定理は最大流最小カット定理に他ならない． \qed
...
\begin{frame}{Mengerの定理（無向・点素ver）}
    ...
    \begin{block}{Mengerの定理（無向・点素ver）}
        ...
        $= \min\{ |U| : U \subseteq V \text{は$s$と$t$を分離} \}$
    \end{block}
    \structure{(証明)} 各枝を両方向に向きづけた有向グラフに対し，有向・点素Mengerを使う． \qed
\end{frame}

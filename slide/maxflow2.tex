...
\begin{frame}{補題の証明}
    ...
    \bigskip
    $X$を残余ネットワーク$G_\varphi$において$s$から到達可能な頂点集合とする．
    すると，$G_\varphi$において$X$から出る枝は存在しないことから，
    以下が成り立つ:
    \begin{itemize}
...
\begin{frame}{Ford--Fulkerson法}
    ...
    \begin{algorithmic}[1]
        ...
        \WHILE {残余ネットワーク$G_\varphi$に増加道$P$が存在}
            \STATE $\eps \leftarrow \min_{a \in P} c_\varphi(a)$ \textcolor{gray}{\footnotesize \dotfill $P$に沿った最大の増加量}
            \STATE $\varphi(a) \gets 
            \begin{cases}
                \varphi(a) \textcolor{UniBlue}{{}+\eps} & a \in P \cap \Aphiup \\
                \varphi(a) \textcolor{AlertOrange}{{}-\eps} & a^{-1} \in P \cap \Aphilow \\
...
\begin{frame}{Edmonds--Karp法の解析}
    ...
    そのような頂点$i$の中で$d^+(i)$が最小のものを取る（\textbf{最小反例}）．
    ...
    \pause
    \bigskip
    $j :=$ $G_{\varphi^+}$の$i$から$t$への最短路における$i$の直後の頂点　とする． \\

    このとき，次が成立:
    \begin{itemize}
        \item $d(j) \leq d^+(j)$ \hfill ($i$の最小性から)
        \item $d^+(i) = d^+(j) + 1$ \hfill ($j$の定義から)
    \end{itemize}
...
\begin{frame}{Edmonds--Karp法の解析}
    ...
    したがって，
    \begin{itemize}
        \item 更新前の$G_\varphi$には$ji$が存在し，その逆向き枝$ij$は存在しない．
        \item 一方，更新後の$G_{\varphi^+}$に$ij$が現れているので，$ji$は$G_\varphi$の最短路$P$に含まれていたことになる． \hfill ∴\highlight{$d(j) = d(i) + 1$}.
    \end{itemize}

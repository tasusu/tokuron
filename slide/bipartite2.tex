...
\begin{frame}{最短路問題}
    ...
    \end{columns}
     \bigskip
     $P$: $s$--$t$路（{\small $s$から$t$へ行く経路で，同じ枝や頂点を複数回通っても良い}） に対し，
     \[
...
\begin{frame}{ポテンシャル}
    ...
    \bigskip\pause
    \textbf{②$\implies$③:} 任意の有向閉路$C: v_0 \to v_1 \to \dots \to v_k = v_0$に対して，
    \[ 
...
\begin{frame}{ポテンシャルの存在と負閉路}
    
    \begin{theorem}
        有向グラフ$G$と枝長$\ell: A \to \R$に対して，\\
        ポテンシャル$p: V \to \R$が存在 $\iff$ 負閉路が存在しない
    \end{theorem}
    \begin{proof}
        $G$に頂点$s$を追加し，$s$から$G$の全ての頂点へ枝を追加したグラフを$G'$とする．
        新しい枝の長さを$0$と定めて$\ell$を拡張した枝長を，$\ell'$とする．このとき，
        \begin{center}
        $(G', \ell')$の負閉路 $\iff$ $(G, \ell)$の負閉路
        \end{center}
...
\begin{frame}{最短路問題のアルゴリズム}
    ...
    \bigskip
    これらは，$s$--$t$最短路だけでなく，実行可能ポテンシャルも同時に求める．
\end{frame}
...
\begin{frame}{主双対法}
    ...
    \begin{block}{主双対法}
        \begin{algorithmic}[1]
            ...
            \STATE $p(i) := 0$ ($i \in V^+$), $p(j) := \min_{i \in \delta(j)} w(ij)$ ($j \in V^-$) \COMMENT{初期ポテンシャル}
            \WHILE{$M$に対する増加道が存在する}
...
\begin{frame}{主双対法の証明}
    ...
    \bigskip
    \structure{Claim.} $p^+$は$(D_M, \ell_M)$のポテンシャル 

    \pause
    {\footnotesize 
        \structure{(∵)} $d$は$(D_M, \ell_{M,p})$のポテンシャルなので，任意の$D_M$の枝$a = ij$に対し，
    \begin{align*}
        d(j) - d(i) \leq \ell_{M,p}(a) = \ell_M(a) + p(i) - p(j)
    \end{align*}
    が成り立つ．移項すると $p^+(j) - p^+(i) \leq \ell_M(a)$.
    }

    \pause
    \bigskip
    \structure{Claim.} $M^+ := M \triangle P$に対し，$p^+$は$(D_{M^+}, \ell_{M^+})$のポテンシャル
    ...
    \end{align*}
    よって，$M$の更新により$P$の枝が反転されても，$p^+$がポテンシャルであることは保たれる． }
    \qed
\end{frame}
...
\begin{frame}{ポテンシャル最適性条件・閉路最適性条件}
    ...
    \begin{proof} \\
    \textbf{①$\iff$②:} $M$が最適 $\iff$ $\exists p$ s.t. $(M, p)$は最適性条件を満たす \\
    \textbf{②$\iff$③:} 最短路問題のところでやった． 
    \end{proof}
\end{frame}
    ...
    \begin{block}{ハンガリー法}
        \begin{algorithmic}[1]
            ...
            \WHILE{\textbf{True}}
                ...
                \ELSE[{ポテンシャル$p$を更新}]
                    \STATE \alt<1>{$X$を$D_M(p)$における$U^+$から到達可能な頂点全体とする．}{$G_p$の最小頂点被覆$(S,T)$を求める \COMMENT{$|S|+|T| < n/2$}}

...
\begin{frame}{単調劣モジュラ関数最大化~\cite{Nemhauser1978}}
    $f : 2^V \to \R_+$ ... 非負\alert{単調}劣モジュラ, {\footnotesize $f(\emptyset)=0$} \\
    \hfill {\small [単調 $\defiff$ $f(X) \leq f(Y)$ \quad ($X \subseteq Y$)]}
...
\begin{frame}{確率的貪欲法~\cite{Mirzasoleiman2015}}
    ...
    \begin{algorithmic}[1]
        \STATE $X_0 \gets \emptyset$
        \FOR{$i=1, \dots, k$}
        \STATE $R_i:= V\setminus X_{i-1}$ から$\frac{n}{k}\log\frac{1}{\eps}$ 個ランダムサンプル
    ...
    \begin{itemize}
        \item
        $
            \E [ f(X)] \geq (1-1/e-\eps) f(X^*)
            $
        \item $O(n\log(1/\eps))$回の関数値評価 \\
...
\begin{frame}{連続貪欲法(離散時間版)}
    ...
    \begin{itemize}
        ...
        \item 精度$\eps>0$に応じて 時間幅$\delta$を十分小さく取れば，同様の仮定のもと $F(\bx) \geq (1-1/e-\eps) \max_{\bx^* \in P}F(\bx^*)$
    \end{itemize}
\end{frame}

...
\begin{frame}{劣モジュラ関数}
    \begin{itemize}[<+->]
        ...
        \item 特に，
    \[
        f(X) + f(Y) = f(X \cup Y) + f(X \cap Y) \qquad (\forall X, Y \subseteq V)
    \]
    ...
    \[
        f(X) + f(Y) \leq f(X \cup Y) + f(X \cap Y) \qquad (\forall X, Y \subseteq V)
    \]
        が成り立つ関数．
    \end{itemize}
...
\begin{frame}{例2: 凹関数と線形関数の合成}
    ...
    \begin{tikzpicture}[>=latex,scale=1.2]
        \draw[->] (0,0) -- (4,0);
        \draw[->] (0,0) -- (0,3);
        \draw[domain=0:4,smooth,variable=\x,thick,UniBlue] plot ({\x},{myfunc(\x)});
        ...
        \draw[->,ultra thick] (a |- 0,0) node[below]{$w(X)$} -- node[midway,above]{$+w_a$} (b |- 0,0);
        \draw[->,ultra thick] (c |- 0,0) node[below]{$w(Y)$} -- node[midway,above]{$+w_a$} (d |- 0,0);
        \draw[->,ultra thick] (a -| 0,0) -- node[midway, left]{$f(a \mid X)$} (b -| 0,0);
        \draw[->,ultra thick] (c -| 0,0) -- node[midway, left]{$f(a \mid Y)$} (d -| 0,0);
        \node[above] at (4, {myfunc(4)}) {$\varphi$};
    \end{tikzpicture}
        \end{column}
    \end{columns}
    \begin{itemize}
        \item $f(X) = \sqrt{w(X)}$ など
    \end{itemize}
\end{frame}
...
\begin{frame}{例3: グラフのカット関数}
    ...
    \begin{block}{}
        \[
            f(X) = u(E[X, \bar X]) \quad (X \subseteq V) 
        \]
    \end{block}
    \end{column}
    \begin{column}{.4\textwidth}
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \node[AlertOrange, above] at ($(a)!.5!(c)$) {$E[X, \bar X]$};
        \begin{scope}[on background layer]
        \node[mysubset, fit=(s) (a) (b),"{$X$}" {below, UniBlue}] {};
        \end{scope}
    \end{tikzpicture}
...
\begin{frame}{限界効用逓減性}
    ...
    \structure{(証明)}
    \textbf{[$\implies$]} 
    $X \cup a$と$Y$に対し，劣モジュラ性を使うと
    \begin{align*}
        f(X \cup a) + f(Y) &\geq f((X \cup a) \cup Y) + f((X \cup a) \cap Y) \\
        &= f(Y \cup a) + f(X).
    \end{align*}
    移項すると限界効用逓減性が出る．
\end{frame}
...
\begin{frame}{基多面体}
    ...
    \begin{definition}
        \begin{align*}
            ...
            B(f) &:= \{ x \in P(f) : x(V) = f(V)\} &\quad &\text{劣モジュラ基多面体}
        \end{align*}
    \end{definition}
    ...
    cf. マトロイド多面体・基多面体（第7回）
        \begin{align*}
            P(\bM) &:= \{ x \in \R_+^E : x(X) \leq r(X) \; (\forall X \subseteq E)\} \\
...
\begin{frame}{Lov\'asz拡張の性質}
    ...
    \begin{column}{.5\textwidth}
        定理より，凸関数$\hat f$の最小化で劣モジュラ関数$f$の最小化ができる． \\
...
\begin{frame}{$L_2$正則化Lov\'asz拡張最小化}
    ...
    $w^*$: (P)の（一意）最適解とする．
    \begin{itemize}
        \item $\{w^* \geq \alpha\}$は(SFM$_\alpha$)の最適解 \\
        \alert{全ての$\alpha$に対し，(SFM$_\alpha$)の最適解が得られる！}

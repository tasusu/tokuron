...
\newcommand{\indepaxiominner}{%
        ...
        \item[(I2)] $I_1, I_2 \in \caI$, $\abs{I_1} < \abs{I_2}$ $\implies$ $\exists e \in I_2 - I_1$~s.t.~$I_1 + e \in \caI$
}
...
\begin{frame}{補題の証明}
    ...
    \bigskip
    \textbf{(I2):} 独立集合$I_1$, $I_2$ ($\abs{I_1} < \abs{I_2}$)を取る．
    
    \bigskip
    \structure{Claim.} $A[I_2]$の列のうち，$A[I_1]$の列が張る部分空間に含まれないものが存在する．\\
    {\small \structure{(∵)} $A[I_1]$, $A[I_2]$の列の張る部分空間をそれぞれ$W_1, W_2$とすると，$\dim W_1 = \abs{I_1} < \abs{I_2} = \dim W_2$であるから．}
    
    \bigskip
    そのような列の添字を$e \in I_2 - I_1$とすると，$I_1 + e$は独立集合．\qed
\end{frame}
...
\begin{frame}{分割マトロイド$\subset$線形マトロイド}
    ...
    \begin{align*}
        ...
        \end{bNiceArray}
    \end{align*}
    を考えると，$A$が定める線形マトロイドの独立集合族は分割マトロイドの独立集合族に一致する．
    \end{column}
    \begin{column}{.2\textwidth}
    \begin{tikzpicture}[yscale=.8]
                \foreach \y in {0,1,2}{
...
\begin{frame}{基族}
    ...
    $B_1, B_2$は独立集合なので，公理(I2)より，ある$e \in B_2 - B_1$で$B_1 + e \in \caI$となるものが存在．これは$B_1$の極大性に反する． \qed
    }
...
\begin{frame}{基族}
    ...
    \begin{description}
        ...
        $\implies \exists j \in B_2 - B_1$~s.t.~$B_1 - i + j \in \caB$.
    \end{description}
    \end{theorem}
    \end{column}
    ...
    \smallskip
    \textbf{(B2):} $B_1 - i$と$B_2$は独立集合で，$\abs{B_1 - i} = \abs{B_1} - 1 < \abs{B_2}$．よって，公理(I2)より，ある$j \in B_2 - (B_1 - i) = B_2 - B_1$で$(B_1 - i) + j \in \caI$となるものが存在．$\abs{B_1 - i + j} = \abs{B_1}$なので，補題よりこれは基．\qed
\end{frame}
...
\begin{frame}{階数関数}
    ...
    \begin{itemize}
        ...
        \item 線形マトロイド: $r(X) = \dim\mathrm{span}\{\text{$A[X]$の列ベクトル}\}$
    \end{itemize}
...
\begin{frame}{定理の証明}
    ...
    \small
    \structure{(証明)} \textbf{(R1), (R2):} 定義から自明．  
...
\begin{frame}<1,8>[label=MST]
    ...
    \begin{center}
        \tikz[scale=2]{
            ...
            \node(a) at (0, 0) {\coloremojicode[scale=2.5]{computer}};
            \node(b) at (1, 1) {\coloremojicode[scale=2.5]{computer}};
            \node(c) at (3, 1) {\coloremojicode[scale=2.5]{computer}};
            \node(d) at (5, 1) {\coloremojicode[scale=2.5]{computer}};
            \node(e) at (6, 0) {\coloremojicode[scale=2.5]{computer}};
            \node(f) at (5,-1) {\coloremojicode[scale=2.5]{computer}};
            \node(g) at (3,-1) {\coloremojicode[scale=2.5]{computer}};
            \node(h) at (1,-1) {\coloremojicode[scale=2.5]{computer}};
            \node(i) at (2, 0) {\coloremojicode[scale=2.5]{computer}};
            % fire
            \node<9-> at (c) {\coloremojicode[scale=2.5]{fire}};
            \end{scope}
...
\begin{frame}{貪欲法}
    ...
    \begin{algorithmic}[1]
        ...
        \FOR{$i = 1, \dots, n$}
            ...
            {\color{gray} \dotfill $X_k$は大きさ$k$の独立集合}
        \ENDFOR
    \end{algorithmic}
    \end{block}
    ...
    \vfill
    \begin{theorem}
        $k = 0, 1, \dots, r(\bM)$に対し，$X_k$が存在し，$X_k$は大きさ$k$の独立集合のうち最小重みである．
    \end{theorem}
\end{frame}
...
\begin{frame}{定理の証明}
    ...
    よって，貪欲法は$e$を$X'$に追加したことになり，$e \in X$となって矛盾．\qed
...
\begin{frame}{定理の証明（続き）}
    ...
    独立集合$\Xmi$と$\Yi$に公理(I2)を適用すると，ある$y \in \Yi - \Xmi$で$\Xmi + y \in \caI$となるものが存在する．
...
\begin{frame}{定理の証明 (1/4)}
    $P := \{x \in \R^E: x(X) \leq r_{\bM}(X) \quad (\forall X \subseteq E), \quad x \geq 0 \}$とする．

    \bigskip
    \structure{$P \subseteq P(\bM)$であること:}
...
\begin{frame}{定理の証明 (2/4)}
    \small
    \structure{$P(\bM) \subseteq P$であること:} 任意の$w \in \R^E$に対し，$\max_{x \in P} w^\top x$を達成する点が$P(\bM)$に存在することを示す．
...
\begin{frame}{定理の証明 (3/4)}
    ...
    \begin{primal}
        ...
        \begin{alignat*}{3}
            \text{max}   \quad & \sum_{e \in E} w(e)x(e) && \quad \\
            \text{s.t.}  \quad & \sum_{e \in X} x(e)  \leq r(X) && \quad (X \subseteq E) \\
    ...
    \begin{dual}
        ...
        \begin{alignat*}{3}
            ...
            \text{s.t.} \quad & \sum_{X \in e} y_X \geq w(e)  &&\quad && (e \in E) \\

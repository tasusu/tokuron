...
\begin{frame}
    ...
    \end{lemma}
    \begin{proof}
        $M$は端点を共有しないので，$M$を被覆するには少なくとも$\abs{M}$個の頂点が必要．
    \end{proof}
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    いま，$\abs\M < \abs\Mp$より，$\M + \Mp$の連結成分の中に，交互道で始点と終点が$\M$の端点でないものが存在する．これは$\M$に対する増加道． 
    \end{proof}
\end{frame}
...
\begin{frame}
    ...
    \begin{proof}<2-> 
        全ての枝は右向きに進めるので，$(S,T)$は定義から頂点被覆．
    また，増加道が存在しないので，$X \cap U^- = \emptyset$.
    ゆえに$(S,T)$の全ての頂点は$M$の枝を1本だけ被覆しているので，$|S|+|T|=|M|$.
    \end{proof}
...
\begin{frame}
    ...
    \begin{column}{.6\textwidth}
        ...
        \begin{enumerate}
            \item $M$は完全マッチング
            \item $y$は(D)の実行可能解
            \item $y_i + y_j = w_e$ \quad ($e = ij \in M$) \\ \hfill \alert{(相補性条件)}
        \end{enumerate}
        \end{lemma}
...
\begin{frame}
    ...
    \pause
    \begin{lemma}
        $D_M(y)$: $G_y$に対する重みなしマッチングで使う有向グラフ\\ 
        $X$: $D_M(y)$における$U^+$から到達可能な頂点全体\\

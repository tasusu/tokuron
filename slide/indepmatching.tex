...
\begin{frame}[label=indepmatching]{独立マッチング}
    \begin{columns}[T]
...
\begin{frame}{独立マッチングの最大最小定理}
    ...
    \begin{proof}
    \[
        \abs{M} \leq \abs{\partial^+ M \cap S^+} + \abs{\partial^- M \cap S^-}  \leq r^+(S^+) + r^-(S^-) 
    \]
    \end{proof}
\end{frame}

\begin{frame}{独立マッチングの最大最小定理}
    \structure{今日の目標:} 以下の最大最小定理をアルゴリズム的に証明し，主・双対最適解が多項式時間で求められることを示す．
    ...
    \vfill
    これは，K\H{o}nig--Egerv\'aryの定理（第２回）の一般化である．
    \small
...
\begin{frame}{マトロイド交差の最大最小定理}
    ...
    \begin{theorem}[Edmonds (1970)]
        \[
            \max_{I \in \caI_1 \cap \caI_2} |I| = \min_{X \subseteq E} r_1(X) + r_2(E \setminus X)
        \]
    \end{theorem}
    ...
    \begin{column}{.7\textwidth}
        \begin{proof}
        帰着に用いた二部グラフにおいて，任意の$X \subseteq E$に対して$(S^+, S^-) = (X, E \setminus X)$は頂点被覆．

        \smallskip
        逆に，任意の頂点被覆$(S^+, S^-)$に対し，$X = S^+$とすると，頂点被覆の定義から$E \setminus  X \subseteq S^-$．
        階数関数の単調性より，$r_2(E \setminus X) \leq r^-(S^-)$なので，右辺の最小化においては$(X, E\setminus X)$の形の頂点被覆だけ考えれば十分． 
        \end{proof}
    \end{column}
    \begin{column}{.3\textwidth}
        \centering
        ...
        \begin{scope}[on background layer]
            ...
            \node[mysubset, fit=(6), "$E \setminus X$" right]{} ;
        \end{scope}
        }
    \end{column}
    \end{columns}
...
\begin{frame}{$k$独立集合問題→独立マッチングへの帰着}
    ...
    \end{columns}
    \begin{block}{}
    大きさ$\abs{E}$の独立マッチング $\longleftrightarrow$ 独立集合への分割$E = I_1 \cup \dots \cup I_k$ 
    \end{block}\qed
\end{frame}
...
\begin{frame}{応用例②: 最小重み全域有向木}
    ...
    \begin{definition}
        有向グラフ$D = (V, A)$の部分グラフ$T$が，$r \in V$を根とする\alert{全域有向木 (spanning arborescence)}\\ 
        $\defiff$
        \begin{itemize}
    ...
    \begin{column}{.3\textwidth}
        ...
        \end{tikzpicture}
        \\
        \footnotesize $r$を根とする全域有向木の例
    \end{column}
    \end{columns}

    \begin{block}{最小重み全域有向木問題}
        $w: A \to \R$: 枝重み
        \[
...
\end{definition}
    ...
    \begin{column}{.7\textwidth}
        \begin{itemize}
            \item $X$が独立集合 $\iff$ $X$はサーキットを含まない．
        \end{itemize}
...
\begin{frame}{定理の証明}
    ...
    \textbf{(C2): } 
    $C_1 \neq C_2$と(C1)から要素$f \in C_1 \setminus C_2$が存在する．
    極小性から$C_1 - f$は独立集合である．
    $C_1 - f$に$C_2 \setminus C_1$の要素を追加して$C_1 \cup C_2$に含まれる最大の独立集合$B$を作る．
    ...
    \centering
        \tikz{
            ...
            \node[point,fill=AlertOrange,"$e$" above] (e) at ($(B1.base)!.5!(B2.base)$) {};
    ...
    \end{columns}
    \[
        \abs{B} < \abs{C_1 \cup C_2} - 1 = \abs{(C_1 \cup C_2) - e}.
    \]
    $B$は$C_1 \cup C_2$に含まれる最大の独立集合であったから，それより大きい$(C_1 \cup C_2)- e$は従属集合であり，したがってサーキットを含む． \qed
\end{frame}
...
\begin{frame}{基本サーキットと交換可能性}
    \begin{lemma}
        $I \in \caI$, $x \in I$, $y \notin I$, $I + y \notin \caI$とする．このとき，$x \in C(I, y) \iff I - x + y \in \caI$.
    \end{lemma}
    ...
    $x \in C(I, y)$より，$I - x + y$は$C(I, y)$を含まないから，独立集合である．逆も同様． 
    \end{proof}
    \end{column}
...
\begin{frame}{逐次交換可能性の証明}
    ...
    \begin{proof}
         $I_k := I \setminus \{x_1, \dots, x_k\} \cup \{y_1, \dots, y_k\}$ ($k=0, \dots, s$)とおく．$I_k \in \caI$を$k$に関する帰納法で示す．
    ...
    \bigskip
    $C := C(I_{k-1}, y_k)$を基本サーキットとする．$x_k \in C$を示せばよい．

    仮定②より$x_1, \dots, x_{k-1} \notin C(I, y_k)$なので，$C(I, y_k) \subseteq I_{k-1} + y_k$. 
    基本サーキットの一意性より$C = C(I, y_k) \underset{\text{①}}{\ni} x_k$.
    \end{proof}
\end{frame}
...
\begin{frame}{補助ネットワーク}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            ...
            \color{UniGreen} A^+ &:= \{(x, y) : \partial^+ M + y \notin \caI^+, x \in C(\partial^+ M, y) - y\} \\
            & \quad \text{\color{gray}\dots\dots$\bM^+$の交換可能性を表す枝} \\
            \color{UniGreen} A^- &:= \{(y, x) : \partial^- M + y \notin \caI^-, x \in C(\partial^- M, y) - y\} \\
            & \quad \text{\color{gray}\dots\dots$\bM^-$の交換可能性を表す枝} \\
        \end{align*}
    \end{itemize}
    \end{column}
...
\begin{frame}{補助ネットワーク}
    ...
    \begin{itemize}
        \item $U^+ := \{i \in V^+ \setminus \partial^+ M : \partial^+ M + i \in \caI^+\}$
        \item $U^- := \{j \in V^- \setminus \partial^- M : \partial^- M + j \in \caI^-\}$
    \end{itemize}
...
\begin{frame}{増加道アルゴリズム}
    \begin{theorem}
        \begin{itemize}
            \item $M$が独立マッチングで，$P$が\alert{枝数最小の}$U^+$--$U^-$パスならば，$M'$は独立マッチング．
            \item $U^+$--$U^-$パスが存在しないとき，$X$を$D_M$において$U^+$から到達可能な頂点集合とすると，$(V^+ \setminus X, V^- \cap X)$は頂点被覆で，かつ強双対性
        \[
            \abs{M} = r^+(V^+ \setminus X) + r^-(V^- \cap X)
        \]
        を満たす．したがって，$M$は最大独立マッチングで，$(V^+ \setminus X, V^- \cap X)$は階数最小頂点被覆である．
        \end{itemize}
    \end{theorem}
\end{frame}
...
\begin{frame}{定理の証明 (1/2)}
    ...
    \begin{block}{}
        \centering
    $\partial^+ M' = (\partial^+ M + i) \setminus \{ x_1, \dots, x_s \} \cup \{ y_1, \dots, y_s \}$ \\
    \end{block}

    $I := \partial^+M + i$と$x_1, \dots, x_s$, $y_1, \dots, y_s$が逐次交換補題の条件を満たすことを確かめればよい． 
    \end{column}
    \begin{column}{.3\textwidth}
        \centering
        \footnotesize
...
\begin{frame}{定理の証明 (2/2)}
    \small
    \textbf{[$(V^+ \setminus X, V^- \cap X)$が階数最小頂点被覆であること]} 
    \begin{columns}[T]
    \begin{column}{.7\textwidth}

    $(S^+, S^-) := (V^+ \setminus X, V^- \cap X)$とする．これが頂点被覆であることは二部マッチングのところで既にやった．

    \onslide<2->{
    \begin{itemize}
        \item $X$から出る$A^+$の枝が存在しないことから，任意の$i \in S^+ \setminus \partial^+ M$に対し，$C(\partial^+ M, i) \subseteq S^+$
        \item 任意の$i \in S^+ \setminus \partial^+ M$に対し，$\partial^+ M \cap S^+ + i$はサーキット$C(\partial^+ M, i)$を含むから，従属集合．
    \end{itemize}
